function [J] = calc_discriminate(x,alpha,T)
% Discriminate model
% x
% T
% alpha
...
J = (trapz(t(T2:T3),y(T2:T3,1)*(alpha-c)) + trapz(t(T3:T4),y(T3:T4,1)*(0.5*alpha-c))...
    + trapz(t(T2:T3),y(T2:T3,2)*(0.5*alpha-c)) + trapz(t(T3:T4),y(T3:T4,2)*(alpha-c)))/(2*T);
end
...
dxdt(1) = (params.Aa + params.B*x(1))*( 1- x(1) - x(2) ) - x(1)*params.G - params.P*params.lagB*x(1)*x(2);
dxdt(2) = (params.Ab + params.B*x(2))*( 1- x(1) - x(2) ) - x(2)*params.G - params.P*params.lagA*x(1)*x(2);
end
